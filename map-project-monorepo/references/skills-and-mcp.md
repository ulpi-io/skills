...
Use this reference when `map-project-monorepo` needs to discover the skills and MCP servers the
workspace provides, and render them into the ROOT `CLAUDE.md` so future agents prefer them instead of
reinventing work or forgetting they exist.

Why this section lives INLINE in the root `CLAUDE.md` (not an imported reference, and not a per-member
file): the root `CLAUDE.md` is always in context; imported references are lazy-loaded (or only pulled
in via `@import`). A "prefer the available skills/MCP" directive only changes behavior if it is always
loaded. These capabilities are workspace-wide, so they belong in root, never duplicated per member.
Keep it compact — a table, not prose.
...
## Output section for the root `CLAUDE.md`

Two pieces, both INLINE in the ROOT `CLAUDE.md` (never a per-member file, never a lazy reference):

1. A one-line pointer at the VERY TOP of the root `CLAUDE.md`, above the project overview, so it is the
   first thing read:

       > ⚡ Check "Available Skills & MCP" below before implementing — prefer existing skills/MCP over
       > doing the work by hand.

2. ONE compact section near the top, after the project overview and before workspace layout / member
   routing:
